  ...
  return await leave.aggregate([
    ...
    },
    {
      $project: {
        ...
        LeaveType: "$leaveType",
        "From Date": {
          $dateToString: { format: "%d-%m-%Y", date: "$FromDate" },
        },
        "To Date": { $dateToString: { format: "%d-%m-%Y", date: "$ToDate" } },
        "Duration(days)": { $add: ["$Duration", 1] },
        Status: "$status",
      },
    },
  ]);
  ...
  return await leave.aggregate([
    {
      $facet: {
        ...
        "Awaiting for Approval": [
          {
            $match: { UserID: acc, status: "pending" },
          },
          { $count: "Total" },
        ],
      },
    },
    {
      $project: {
        " CL  Availble": {
          $subtract: [
            12,
            { $ifNull: [{ $arrayElemAt: ["$Total count CL.Total", 0] }, 0] },
          ],
        },
        "EL  Availble": {
          $subtract: [
            20,
            { $ifNull: [{ $arrayElemAt: ["$Total count EL.Total", 0] }, 0] },
          ],
        },
        "Total WFH Taken": {
          $ifNull: [{ $arrayElemAt: ["$Total count WFH.Total", 0] }, 0],
        },
        "Total LOP Taken": {
          $ifNull: [{ $arrayElemAt: ["$Total count LOP.Total", 0] }, 0],
        },
        "Available leave": {
          $subtract: [
            32,
            { $ifNull: [{ $arrayElemAt: ["$Available leave.Total", 0] }, 0] },
          ],
        },
        "Awaiting for Approval": {
          $ifNull: [{ $arrayElemAt: ["$Awaiting for Approval.Total", 0] }, 0],
        },
      },
    },
  ]);
